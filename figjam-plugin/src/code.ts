...
function parseComponent(value: unknown, path: string): UniMapComponent {
  if (!isRecord(value)) {
    throw new Error(`${path} must be an object.`);
  }
...
async function renderDocument(document: UniMapDocument): Promise<void> {
  ...
  let sceneSection: SectionNode | undefined;
  try {
...
function renderHierarchyObject(
  ...
  const width = Math.max(
    BASE_NODE_WIDTH,
    componentRight > 0 ? componentRight + PADDING : 0,
    widestChild > 0 ? widestChild + PADDING * 2 : 0,
  );
...
function getHierarchyFill(hierarchyObject: UniMapHierarchyObject): SolidPaint {
